 ...
 * @version    $Id: RedisCluster.class.php 2016年3月17日 13:01:41Z $  
 */ 
class RedisRac{
     ...
     * @return boolean
     */
    public function connect($config=array('host'=>'127.0.0.1','port'=>6379,'auth'=>''), $isMaster=true){
        // default port
        ...
        if($isMaster){
            ...
            $this->ret = $this->_linkHandle['master']->pconnect($config['host'],$config['port']);
            if(!empty($config['auth']) && $this->ret)  $this->ret = $this->_linkHandle['master']->auth($config['auth']);
        }else{
            // 多个 Slave 连接
            $this->_linkHandle['slave'][$this->_sn] = new Redis();
            $this->ret = $this->_linkHandle['slave'][$this->_sn]->pconnect($config['host'],$config['port']);
            if(!empty($config['auth']) && $this->ret) $this->ret = $this->_linkHandle['slave'][$this->_sn]->auth($config['auth']);
            if($this->ret) ++$this->_sn;
        }
        $this->ret or die('redis link failed!');
        return $this->ret;
    }
...
*/
    ...
    //为KEY 设置过期时间
    public function setExpire($key,$expire=3600){
       $t =   $this->getRedis()->ttl($key);
       if( $t> $expire || $t== '-1') $this->getRedis()->expire($key,$expire);        
       return  true;
    }
    
       //获取所有的key, $keys 为通配符
    public function keys($keys='*'){
       return $this->getRedis()->keys($keys);    
    }  
    ...
    public function aset($name,$value,$expire=0){        
        if(is_array($value)){
            return $this->set($name,json_encode($value,JSON_UNESCAPED_UNICODE),$expire);    
        }
    ...
    public function aget($name){      
        $row = $this->get($name);
        if($row){      
               //$row   = preg_replace('!s:(\d+):"(.*?)";!se', '"s:".strlen("$2").":\"$2\";"',$row);        
     ...
     *    set hash opeation
     */
    public function hset($name,$key,$value,$timeout=0){        
        if($timeout >0 ) $this->getRedis()->expire($name, $timeout);
        if(is_array($value)){
            return $this->getRedis()->hset($name,$key,json_encode($value,JSON_UNESCAPED_UNICODE));    
        }
